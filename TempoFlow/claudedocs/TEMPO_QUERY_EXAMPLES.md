...
## 🔍 Multi-Namespace Service Error Query
...
## ⚡ Multi-Namespace High Latency Query
...
**Generated By**: Node 1 (Unified Entry Point) → Node 4 (Service Detection)
...
2. **Invalid Attribute Paths**:
   - `.status.code` ❌ (leading dot) → `status.code` ✅
...
1. Navigate to: **Explore** → **Tempo** datasource
